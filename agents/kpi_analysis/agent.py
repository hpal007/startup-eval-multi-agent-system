...
def analyze_kpi_performance_tool(startup_data: str) -> str:
    ...
    """
    try:
        data = json.loads(startup_data) if isinstance(startup_data, str) else (startup_data or {})
        ...
        def _to_float(val):
            ...
            try:
                ...
                return None
        current_kpis = {k: (_to_float(v) if isinstance(v, str) else v) for k, v in current_kpis.items()}
        ...
        return json.dumps({"error": f"Failed to analyze KPI performance: {str(e)}"})
...
def generate_recommendations_tool(analysis_data: str) -> str:
    ...
    """
    try:
        data = json.loads(analysis_data) if isinstance(analysis_data, str) else (analysis_data or {})
        
        # Accept either nested or flat shape
        performance_results = data.get('performance_results', data.get('results', data)) or {}
...
def identify_advantages_tool(performance_data: str) -> str:
    ...
    try:
        ...
        advantages = identify_competitive_advantages(performance_results, industry_context)
        
        return json.dumps(advantages, indent=2)
        
    except Exception as e:
        return json.dumps({"error": f"Failed to identify advantages: {str(e)}"})
...
def validate_claims_tool(validation_data: str) -> str:
    ...
    try:
        ...
        return json.dumps({"error": f"Failed to validate claims: {str(e)}"})
...
def analyze_individual_kpi_tool(kpi_analysis_data: str) -> str:
    ...
    """
    try:
        data = json.loads(kpi_analysis_data) if isinstance(kpi_analysis_data, str) else (kpi_analysis_data or {})
        ...
        analysis_results = analyze_individual_kpi_performance(
            kpi_name, current_value, historical_data, industry_benchmarks, industry_context
        )
        
        return json.dumps(analysis_results, indent=2)
        
    except Exception as e:
        return json.dumps({"error": f"Failed to analyze individual KPI: {str(e)}"})
