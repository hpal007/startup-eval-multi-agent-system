...
def analyze_kpi_trends(
    historical_data: dict[str, list[dict]], industry_benchmarks: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    for kpi_name, data_points in historical_data.items():
        ...
        industry_growth = industry_benchmarks.get(kpi_name, {}).get(
            "typical_growth_rate", 0
        )
        relative_performance = (
            "above_industry" if growth_rate > industry_growth else "below_industry"
        )
...
def calculate_performance_scores(
    ...
    return {
        ...
        "total_kpis_analyzed": len(
            [k for k in performance_results.values() if k["score"] is not None]
        ),
    }
    ...
    for kpi_name, kpi_data in individual_scores.items():
        ...
        if status in ["poor", "below_average"]:
            ...
            recommendation = {
                ...
                "recommendations": best_practices[:3]
                if best_practices
                else [
...
def identify_competitive_advantages(
    performance_results: dict[str, Any], industry_context: dict[str, Any]
) -> list[dict[str, Any]]:
    """
    ...
    for kpi_name, kpi_data in individual_scores.items():
        ...
        if percentile >= 90:
            advantage = {
                ...
                "sustainability": assess_advantage_sustainability(
                    kpi_name, kpi_data, industry_context
                ),
...
def assess_advantage_sustainability(
    kpi_name: str, kpi_data: dict[str, Any], industry_context: dict[str, Any]
) -> str:
...
def validate_exceptional_claims(
    kpi_data: dict[str, Any], industry_benchmarks: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    for kpi_name, performance in kpi_data.items():
        if performance.get("percentile", 0) >= 90:
            ...
            validation = {
                ...
                "red_flags": identify_performance_red_flags(
                    performance, benchmark_data
                ),
            }
...
def calculate_validation_confidence(
    performance: dict[str, Any], benchmark_data: dict[str, Any]
) -> float:
...
def identify_performance_red_flags(
    performance: dict[str, Any], benchmark_data: dict[str, Any]
) -> list[str]:
    ...
    if percentile > 99:
        red_flags.append(
            "Performance appears too good to be true - requires verification"
        )
...
def analyze_individual_kpi_performance(
    kpi_name: str,
    current_value: float,
    ...
    percentile = (
        calculate_percentile_ranking(current_value, benchmarks) if benchmarks else None
    )
    ...
    if percentile and percentile >= 75:
        competitive_advantage = {
            ...
            "sustainability": assess_advantage_sustainability(
                kpi_name, {"percentile": percentile}, industry_context
            ),
            "strategic_value": assess_strategic_value(kpi_name, industry_context),
        }
    ...
    analysis = {
        ...
        "benchmark_comparison": {
            ...
            "current_vs_median": (
                (current_value - benchmarks.get("p50", 0)) / benchmarks.get("p50", 1)
            )
            * 100
            if benchmarks.get("p50")
            else None,
        },
        ...
        "risk_factors": identify_kpi_risk_factors(
            kpi_name, current_value, trend_analysis, benchmarks
        ),
        "confidence_score": calculate_analysis_confidence(
            historical_data, benchmarks, industry_context
        ),
    }
...
def generate_kpi_specific_recommendations(
    kpi_name: str,
    current_value: float,
            ...
            recommendations.append(
                {
                    ...
                    "specific_steps": get_industry_specific_improvement_steps(
                        kpi_name, industry, growth_stage
                    ),
                    "expected_impact": "Significant performance improvement",
                    "timeline": "1-3 months",
                    "success_metrics": [
                        f"Achieve {kpi_name} above 25th percentile",
                        f"Improve {kpi_name} by 20-30%",
                    ],
                }
            )
        elif percentile < 50:
...
def get_industry_specific_improvement_steps(
    kpi_name: str, industry: str, growth_stage: str
) -> list[str]:
    """
    ...
    kpi_steps = industry_strategies.get(
        kpi_name.lower(),
        [
...
def calculate_improvement_potential(
    ...
    potential_to_median = (
        ((p50 - current_value) / current_value) * 100 if current_value > 0 else 0
    )
    potential_to_75th = (
        ((p75 - current_value) / current_value) * 100 if current_value > 0 else 0
    )
    potential_to_90th = (
        ((p90 - current_value) / current_value) * 100 if current_value > 0 else 0
    )
