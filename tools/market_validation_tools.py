...
def calculate_market_variance(
    claimed_value: float, market_estimate: float
) -> dict[str, float]:
    """
...
def assess_data_recency(source_date: str, current_date: str | None) -> dict[str, any]:
    """
...
def calculate_confidence_score(
    ...
    multiple_source_confirmation: bool,
) -> dict[str, float]:
    """
...
def identify_discrepancy_flags(
    tam_variance: float | None,
    sam_variance: float | None,
    som_variance: float | None,
    ...
    if tam_variance is not None and sam_variance is not None:
        if (
            tam_variance < sam_variance - 10
        ):  # SAM should not be much higher variance than TAM
            flags.append(
                "SAM variance significantly higher than TAM - potential methodology issue"
            )

    if sam_variance is not None and som_variance is not None:
        if (
            sam_variance < som_variance - 10
        ):  # SOM should not be much higher variance than SAM
            flags.append(
                "SOM variance significantly higher than SAM - potential methodology issue"
            )
    ...
    if variances:
        ...
        if avg_variance > 30:
            flags.append(
                "Average market size variance exceeds 30% - high validation risk"
            )

    return flags


def extract_market_data_from_search(
    search_results: str, market_type: str
) -> dict[str, any]:
    """
    ...
    patterns = {
        ...
        "year": [r"(20\d{2})", r"by (20\d{2})", r"in (20\d{2})"],
        "source": [
            r"according to ([^,.\n]+)",
    ...
    try:
        ...
        confidence_factors = [
            ...
            any(
                "gartner" in s.lower() or "forrester" in s.lower()
                for s in extracted_data["sources"]
            ),
        ]
...
def validate_tam_sam_som_logic(
    tam: float | None, sam: float | None, som: float | None
) -> dict[str, any]:
    """
    ...
    try:
        ...
        if len(provided_values) < 2:
            validation_result["warnings"].append(
                "Insufficient data for relationship validation"
            )
        ...
        if tam is not None and sam is not None:
            if sam > tam:
                validation_result["errors"].append(
                    f"SAM ({sam:.2f}B) cannot be greater than TAM ({tam:.2f}B)"
                )
                validation_result["is_valid"] = False
            else:
                validation_result["relationships"]["SAM_to_TAM_ratio"] = round(
                    sam / tam, 3
                )
        ...
        if sam is not None and som is not None:
            if som > sam:
                validation_result["errors"].append(
                    f"SOM ({som:.2f}B) cannot be greater than SAM ({sam:.2f}B)"
                )
                validation_result["is_valid"] = False
            else:
                validation_result["relationships"]["SOM_to_SAM_ratio"] = round(
                    som / sam, 3
                )
        ...
        if tam is not None and som is not None:
            if som > tam:
                validation_result["errors"].append(
                    f"SOM ({som:.2f}B) cannot be greater than TAM ({tam:.2f}B)"
                )
                validation_result["is_valid"] = False
            else:
                validation_result["relationships"]["SOM_to_TAM_ratio"] = round(
                    som / tam, 3
                )
        ...
        if "SAM_to_TAM_ratio" in validation_result["relationships"]:
            ...
            if ratio < 0.1:
                validation_result["warnings"].append(
                    f"SAM is only {ratio * 100:.1f}% of TAM - unusually small serviceable market"
                )
            elif ratio > 0.8:
                validation_result["warnings"].append(
                    f"SAM is {ratio * 100:.1f}% of TAM - unusually large serviceable market"
                )
        ...
        if "SOM_to_SAM_ratio" in validation_result["relationships"]:
            ...
            if ratio < 0.05:
                validation_result["warnings"].append(
                    f"SOM is only {ratio * 100:.1f}% of SAM - very conservative obtainable market"
                )
            elif ratio > 0.5:
                validation_result["warnings"].append(
                    f"SOM is {ratio * 100:.1f}% of SAM - aggressive obtainable market projection"
                )
