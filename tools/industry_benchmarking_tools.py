...
def calculate_percentile_ranking(
    kpi_value: float, benchmark_data: dict[str, float], kpi_name: str
) -> dict[str, Any]:
    """
    ...
    is_lower_better = any(
        keyword in kpi_name.lower() for keyword in lower_is_better_kpis
    )
    ...
    return {
        ...
        "benchmark_data_used": {"p25": p25, "p50": p50, "p75": p75, "p90": p90},
    }


def analyze_performance_gaps(
    startup_kpis: dict[str, float], kpi_framework: dict[str, Any], growth_stage: str
) -> dict[str, Any]:
    """
        ...
        if startup_value is not None:
            ...
            if stage_benchmarks:
                ...
                kpi_performance = {
                    ...
                    "weighted_score": (percentile_result["percentile"] or 0)
                    * importance_weight,
                }
...
def generate_improvement_recommendations(
    performance_gaps: dict[str, Any], industry: str, growth_stage: str
) -> list[dict[str, Any]]:
    """
    ...
    for gap in performance_gaps.get("critical_gaps", []):
        ...
        recommendations.extend(
            _get_kpi_specific_recommendations(
    ...
    recommendations.extend(
        _get_growth_stage_recommendations(
...
def calculate_industry_position(
    overall_score: float, kpi_performances: list[dict[str, Any]], industry: str
) -> dict[str, Any]:
    """
    ...
    if overall_score >= 80:
        ...
        position_description = (
            "Significant performance gaps compared to industry standards"
        )

    # Calculate performance distribution
    excellent_count = sum(
        1 for kpi in kpi_performances if kpi.get("percentile", 0) >= 90
    )
    good_count = sum(
        1 for kpi in kpi_performances if 75 <= kpi.get("percentile", 0) < 90
    )
    average_count = sum(
        1 for kpi in kpi_performances if 50 <= kpi.get("percentile", 0) < 75
    )
    ...
    return {
        ...
        "performance_distribution": {
            "excellent": {
                ...
                "percentage": excellent_count / total_kpis * 100
                if total_kpis > 0
                else 0,
            },
            "good": {
                "count": good_count,
                "percentage": good_count / total_kpis * 100 if total_kpis > 0 else 0,
            },
            "average": {
                "count": average_count,
                "percentage": average_count / total_kpis * 100 if total_kpis > 0 else 0,
            },
            "poor": {
                "count": poor_count,
                "percentage": poor_count / total_kpis * 100 if total_kpis > 0 else 0,
            },
        },
...
def _find_matching_kpi_value(
    startup_kpis: dict[str, float], target_kpi: str
) -> float | None:
    ...
    # Try partial matches
    for kpi_name, value in startup_kpis.items():
        if target_lower in kpi_name.lower() or kpi_name.lower() in target_lower:
            return value
...
def _get_kpi_specific_recommendations(
    ...
    # Operational efficiency
        ...
        keyword in kpi_lower for keyword in ["margin", "efficiency", "productivity"]
    ):
        recommendations.append(
            {
...
def _get_growth_stage_recommendations(
    growth_stage: str, overall_score: float
) -> list[dict[str, Any]]:
...
def _get_industry_context(industry: str, position: str) -> str:
    ...
    base_context = industry_contexts.get(
        industry.lower(), "Industry-specific performance benchmarks vary significantly"
    )
...
def validate_exceptional_performance(
    ...
    percentile_result = calculate_percentile_ranking(
        kpi_value, benchmark_data, kpi_name
    )
...
def correlate_market_trends(
    ...
    for trend in market_trends:
        ...
        if any(
            ...
            for keyword in ["growth", "expansion", "increasing", "rising"]
        ):
            growth_kpis = _find_growth_related_kpis(startup_kpis)
            for kpi_name, kpi_value in growth_kpis.items():
                correlation = _assess_trend_kpi_correlation(
                    kpi_name, kpi_value, trend, "positive"
                )
...
def generate_typical_ranges_context(
    ...
    return {
        ...
        "improvement_trajectory": _generate_improvement_trajectory(
            kpi_name, growth_stage
        ),
    }
...
def _assess_trend_kpi_correlation(
    ...
    if trend_type == "positive" and "growth" in kpi_name.lower():
        ...
        trend_type == "efficiency"
        and any(
            keyword in kpi_name.lower() for keyword in ["margin", "cost", "efficiency"]
        )
    ) or (
        trend_type == "competitive"
        and any(
            keyword in kpi_name.lower() for keyword in ["retention", "share", "nps"]
        )
    ):
...
def _generate_strategic_implications(
    ...
    if growth_stage == "seed":
        implications.append(
            "Focus on validating product-market fit while monitoring emerging trends"
        )
    elif growth_stage == "growth":
...
def _generate_stage_specific_expectations(
    ...
    return stage_expectations.get(
        growth_stage, f"Target performance above industry median ({p50})"
    )
...
def _generate_industry_specific_context(
    ...
    base_context = industry_contexts.get(
        industry.lower(), "Industry-specific benchmarks vary significantly"
    )
...
def _generate_benchmark_interpretation(kpi_name: str) -> str:
    ...
    if any(keyword in kpi_lower for keyword in ["cost", "churn", "burn"]):
        ...
        keyword in kpi_lower for keyword in ["revenue", "growth", "retention", "margin"]
    ):
...
def _generate_improvement_trajectory(kpi_name: str, growth_stage: str) -> str:
    ...
    return trajectories.get(
        growth_stage, "Establish consistent measurement and improvement processes"
    )
