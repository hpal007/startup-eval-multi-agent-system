...
class Config:
    ...
    def validate_required_configs(cls) -> None:
        ...
        if not cls.OPENAI_API_KEY and (cls.GEMINI_MODEL.startswith('gpt') or cls.OPENAI_MODEL.startswith('gpt')):
            required_configs.append('OPENAI_API_KEY')
        
        if required_configs:
            raise ValueError(f"Missing required environment variables: {', '.join(required_configs)}")
